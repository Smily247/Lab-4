...
int Partition(int *arr, int start, int last) {
    ...
    int i = start - 1; // index of small element
    ...
    // swap the last value in the array with the value just after i
...
// you are finished when there is only 1 left
    if (start < last) {
        // partion the array into 2 parts
...
#pragma omp single
    ...
    printf("Took %f seconds\n\n", end.tv_sec - start.tv_sec + (double) (end.tv_usec - start.tv_usec) / 1000000);
    printf("First 10 numbers in sorted array: ");
    for(int i = 0; i < 10; i++)
        printf("%i, ", arr_p[i]);
    free(arr_p);
